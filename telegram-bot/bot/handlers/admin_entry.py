...
async def trigger_test_notification(message: Message):
    ...
    from ..scheduler import _check_services

    # Run the real notification logic in dry-run mode so dedup timestamps
    # are not recorded and real notifications are not suppressed later.
    await _check_services(message.bot, dry_run=True)
